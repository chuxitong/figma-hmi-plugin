...
- Server `GET /health` → **`ui2coden_prompt_profile` = `'baseline'`** (must match intentional `UI2CODEN_PROMPT_PROFILE` restart for baseline vs extended).
...
- Image-only vs adding variables: `+0.01333`.
- Adding CSS on top of variables: `-0.01850`.
