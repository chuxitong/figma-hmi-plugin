...
本目录（相对仓库根）：`reports/deliverables_week8_prompt_extended/reproducibility_complete`
对应周次：**Week 8**
...
- **服务端追溯目录（每次 Week 7/8 主流程自动发送 `X-Experiment-Trace-Dir`；可用 `--trace-dir` 改根路径）**：仓库内相对路径 `reports/deliverables_week8_prompt_extended/reproducibility_logs/WEEK8_FIXED_SERVER_TRACE_ROOT`。
